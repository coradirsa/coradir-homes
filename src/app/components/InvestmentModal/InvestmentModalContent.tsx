...
interface InvestmentModalContentProps {
  onClose: () => void;
  whatsappNumber: string;
  trackEvents?: boolean;
}

export default function InvestmentModalContent({
  onClose,
  whatsappNumber,
  trackEvents = true,
}: InvestmentModalContentProps) {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = (data: InvestmentFormData) => {
    ...
    const mensaje = `Hola! Vengo del pop-up de inversiones de la web homes.
...
`;
  ...
  return (
    <div
      ...
      className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[680px] max-h-[88vh] overflow-y-auto"
    >
      {/* Close button */}
      <button
        id="investment-modal-close-btn"
        onClick={onClose}
        aria-label="Cerrar modal (ESC)"
        className="absolute top-3 right-3 z-10 text-gray hover:text-black transition-colors text-4xl leading-none p-2"
      >
        ×
      </button>
      ...
      <div className="p-7 md:p-8">
        <h2
          ...
          className="text-2xl md:text-3xl font-playfair font-bold text-blue mb-3"
        >
        ...
        >
          ...
          <div className="pt-2">
            <button
              id="investment-modal-submit-btn"
              type="submit"
