...
export default function ContactForm({ data }: ContactFormProps) {
  const [formType, setFormType] = useState(data.contact.options[0]);
  const [profileType, setProfileType] = useState(data.contact.profileTypes[0].value);

  return (
    <section id="contact" className="relative bg-slate-100 grid grid-cols-1 lg:grid-cols-2 min-h-[700px]">
      <div className="relative h-64 lg:h-auto">
        <Image
          src={data.contact.bgImage}
          alt="Interior del local comercial CORADIR"
          fill
          className="object-cover"
        />
        <div className="absolute inset-0 bg-slate-900/60 flex items-center justify-center p-12 text-center">
          <div>
            <h2 className="font-serif text-4xl md:text-6xl text-white font-bold leading-tight mb-4">
              {data.contact.title}
            </h2>
            <span className="text-xl font-sans font-light text-gray-200">
      ...
      <div className="p-8 md:p-20 bg-white flex flex-col justify-center" id="venta">
        <form className="space-y-6 max-w-lg mx-auto w-full">
          ...
          {data.contact.fields.map((field, idx: number) => (
            <div key={idx}>
              {field.type === 'textarea' ? (
                <textarea
                  name={field.name}
          ...
          <div className="py-2">
            ...
            <div className="flex gap-6">
              {data.contact.profileTypes.map((type, idx) => (
          ...
          <div className="grid grid-cols-2 gap-4 mt-8 pt-6" id="alquiler">
            {data.contact.options.map((opt, idx) => (
              <button
                key={idx}
                type="button"
